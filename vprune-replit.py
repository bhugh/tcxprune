 ...
 * To avoid typing long filenames, rename your .tcx to a short, simple name
	...
	while True:
		...
		if not isinstance(inputfilename, str) or len(inputfilename)==0 or gui==True:
			...
			#print('WARNING!!! All current .tcx files in the repl.it directory with this prefix will be deleted!\n') #repl.it
			#print('WARNING!!! Files matching pattern %s*.tcx in the current repl.it directory will be deleted!'%prefix) #repl.it
			#sys.stderr.flush()
		...
		if gui:
				...
				old_stdout = sys.stdout
				sys.stdout = mystdout = StringIO()
				delete_current_tcx_in_directory(inputfilename, prefix)
				...
				#main_window.Close()
				#sg.Popup("VPrune - Completed!", "Files Processed!\nFiles will be listed in repl.it on the files list to your left.\n")	
				print ('\n\n\n********************\nFiles Processed!\n\nFiles will be listed folder TCXResults in repl.it on the files list to your left')
				
				print('\nClick the three dots @ the top of repl.it Files pane to "download as zip"\n\n********************\n\n\n\n')		
				#raise SystemExit
				os._exit(1)
				break
				time.sleep(0.05)
			...
			process_file_segments (tree, root, inputfilename, maxturns, split, maxpoints, percent, closeancourse, cleannotes, trimnotes)
			break
	#if gui:
		#main_window.Close()
